...
//  val u_tlb = Module(new BRAMTLB(tlbParam, tlbLRUCore))
//
...
//  u_cache.frontend_request.bits.addr := Cat(
...
//  )
//
//  u_cache.frontend_request.bits.asid := frontend_request_i.bits.asid
//  u_cache.frontend_request.bits.wData := frontend_request_i.bits.wData
//  u_cache.frontend_request.bits.wMask := frontend_request_i.bits.wMask
//  u_cache.frontend_request.bits.permission := frontend_request_i.bits.permission
//  u_cache.frontend_request.bits.tid := frontend_request_i.bits.tid
//  u_cache.frontend_request.valid := u_tlb.frontend_reply_o.valid &&
//    u_tlb.frontend_reply_o.bits.hit && // TLB hit
//    !u_tlb.frontend_reply_o.bits.violation // No violation
//
//  frontend_request_i.ready := u_tlb.frontend_request_i.ready && u_cache.frontend_request.ready
//
...
//    val miss_request_i = IO(Flipped(Decoupled(new MissRequestPacket(param.databankParameter))))
//    val backend_reply_i = IO(Flipped(Decoupled(UInt(param.blockSizeInBit.W))))
//    val refill_o = IO(Decoupled(new MissResolveReplyPacket(param.databankParameter)))
...
//
//    val cache_backend_request_i = IO(Flipped(Decoupled(new MergedBackendRequestPacket(param.toCacheParameter.databankParameter))))
//
...
//
//    val cache_backend_reply_o = IO(Decoupled(new MissResolveReplyPacket(param.toCacheParameter.databankParameter)))
//
//    // AXI Read request

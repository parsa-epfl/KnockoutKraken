...
class QEMUMessageDecoder(
  ...
  val qemu_miss_reply_o = IO(Decoupled(new QEMUMissReply(param.mem.toTLBParameter)))
  ...
  val qemu_evict_reply_q = Wire(Decoupled(new QEMUEvictReply(param.mem.toTLBParameter)))
  ...
  val qemu_evict_page_req_q = Wire(Decoupled(new QEMUPageEvictRequest(param.mem.toTLBParameter)))

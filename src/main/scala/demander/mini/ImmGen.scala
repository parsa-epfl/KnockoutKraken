...
class ImmGenIO(implicit p: MiniConfig) extends CoreBundle()(p) {
  val inst = Input(UInt(xlen.W))
  val sel  = Input(UInt(3.W))
  val out  = Output(UInt(xlen.W))
}

abstract class ImmGen(implicit p: MiniConfig) extends Module {
  val io = IO(new ImmGenIO)
}

class ImmGenWire(implicit p: MiniConfig) extends ImmGen()(p) {
...
class ImmGenMux(implicit p: MiniConfig) extends ImmGen()(p) {

...
abstract class CoreBundle(implicit val p: MiniConfig) extends Bundle with CoreParams

class HostIO(implicit p: MiniConfig) extends CoreBundle()(p) {
  val fromhost = Flipped(Valid(UInt(xlen.W)))
  val tohost   = Output(UInt(xlen.W))
}

class CoreIO(implicit p: MiniConfig) extends CoreBundle()(p) {
  val host = new HostIO
  val icache = Flipped((new CacheIO))
  val dcache = Flipped((new CacheIO))
}

class Core(implicit val p: MiniConfig) extends Module with CoreParams {

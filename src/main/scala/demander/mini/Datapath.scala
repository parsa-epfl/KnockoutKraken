...
class DatapathIO(implicit p: MiniConfig) extends CoreBundle()(p) {
  val host = new HostIO
...
class Datapath(implicit val p: MiniConfig) extends Module with CoreParams {
  val io      = IO(new DatapathIO)
  val csr     = Module(new CSR)
  val regFile = Module(new RegFile) 
  val alu     = p.BuildALU() //p(BuildALU)(p)
  val immGen  = p.BuildImmGen()
  val brCond  = p.BuildBrCond()
  ...
  val pc   = RegInit((p.initialPC - 4).S(xlen.W).asUInt())

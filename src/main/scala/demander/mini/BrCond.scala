...
class BrCondIO(implicit p: MiniConfig) extends CoreBundle()(p) {
...
abstract class BrCond(implicit p: MiniConfig) extends Module {
  val io = IO(new BrCondIO)
}

class BrCondSimple(implicit p: MiniConfig) extends BrCond()(p) {
...
class BrCondArea(implicit val p: MiniConfig) extends BrCond()(p) with CoreParams {

...
class ALUIo(implicit p: MiniConfig) extends CoreBundle()(p) {
...
abstract class ALU(implicit val p: MiniConfig) extends Module with CoreParams {
  val io = IO(new ALUIo)
}

class ALUSimple(implicit p: MiniConfig) extends ALU()(p) {
...
class ALUArea(implicit p: MiniConfig) extends ALU()(p) { 

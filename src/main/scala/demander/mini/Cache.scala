...
class CacheReq(implicit p: MiniConfig) extends CoreBundle()(p) {
  val addr = UInt(xlen.W)
  val data = UInt(xlen.W)
  val mask = UInt((xlen/8).W)
}

class CacheResp(implicit p: MiniConfig) extends CoreBundle()(p) {
  val data = UInt(xlen.W)
}

class CacheIO (implicit val p: MiniConfig) extends Bundle {
  val abort = Input(Bool())
  val req   = Flipped(Valid(new CacheReq))
  val resp  = Valid(new CacheResp)
}

// class CacheModuleIO(implicit val p: MiniConfig) extends Bundle {
//   val cpu   = new CacheIO

...
TEST_CASE("ldst-signed-word-pair") {
    FPGAContext ctx;
    DevteroflexArchState state;
    REQUIRE(initFPGAContext(&ctx) == 0);
    initArchState(&state, 0x0);
    ...
    uint8_t page[PAGE_SIZE] = {0}; 
    makeDeadbeefPage(page, PAGE_SIZE);
    ((uint32_t *) page)[0] = 0x69403667; // ldrpsw  x7, x13, [x19]
    ((uint32_t *) page)[1] = 0x0;        // Trigger transplant
    state.xregs[7] = 0xB00B5;
    state.xregs[13] = 0xB00B5;
    state.xregs[19] = 0x0000000001beb320;
    uint64_t vaddr = state.xregs[19];
